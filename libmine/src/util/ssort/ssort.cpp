...
/*
	...
	Modified for the error miner: the second argument (int s[]) is
	dropped, the function accepts a pointer to a std::vector<int>
	as its argument. Macros were replaced by functions.
	...
	for(h = 0; ; h = h == 0 ? 1: 2*h) {
		...
		if(h == 0) {			/* find k */
			...
			for(i=k; i<n; i++)	/* check input */
				...
				{
					cout << p[i] << endl;
					throw domain_error("Incorrect input (e.g. does every hash code 0..k occur?)!");
				}
		}
